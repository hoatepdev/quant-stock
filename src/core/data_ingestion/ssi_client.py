"""SSI iBoard API client for fetching Vietnam stock market data."""
import time
...
class SSIClient:
    """Client for SSI iBoard API with rate limiting and caching."""
    ...
    def __init__(
        ...
        """
        self.api_key = api_key or settings.SSI_API_KEY
        self.secret_key = secret_key or settings.SSI_SECRET_KEY
    ...
    async def __aenter__(self) -> "SSIClient":
    ...
    def _get_cache_key(self, endpoint: str, params: Dict[str, Any]) -> str:
        ...
        return f"ssi:{endpoint}:{param_str}"
    ...
    async def _make_request(
        ...
        """Make HTTP request to SSI API with retry logic.
        ...
        Raises:
            SSIAPIError: If API request fails
        """
        params = params or {}
        ...
        try:
            ...
            raise SSIAPIError(f"API request failed: {e}")
        except Exception as e:
            logger.error(f"Request error: {e}")
            raise SSIAPIError(f"Failed to fetch data: {e}")
    ...
    async def get_daily_prices(
        ...
        data = await self._make_request("historical-data", params)
    ...
    async def get_financial_statements(
        ...
        data = await self._make_request("financial-statement", params)
    ...
    async def get_stock_list(self, exchange: Optional[str] = None) -> List[Dict[str, str]]:
        ...
        data = await self._make_request("stock-list", params, use_cache=True)
    ...
    async def get_corporate_actions(
        ...
        data = await self._make_request("corporate-actions", params)
        ...
        if data and "data" in data:
            for item in data["data"]:
                actions.append({
                    ...
                    "dividend_amount": Decimal(str(item.get("dividend", 0))),
